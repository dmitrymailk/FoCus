...
def run():
    ...
    parser.add_argument(
        "--test_dataset_path",
        type=str,
        default="/home/mnt/yoonna/FoCus_v1/test_focus.json",
        help="Path or url of the dataset. If empty download from S3.",
    )  # data
    parser.add_argument(
        "--test_dataset_cache",
        type=str,
        default="/home/mnt/yoonna/FoCus_v1/focus_cache.tar.gz",
    ...
    with torch.no_grad():
        ...
        for test_data_index, test_data in enumerate(tqdm(test_loader)):
            ...
            if model.config.model_type == "gpt2":
                ...
                k_index_cvtd = torch.tensor(
                    [1 if num in k_index_1 else 0 for num in range(10)],
                    device=args.device,
                )
                ...
                for batch_i in range(args.test_batch_size):
                    ...
                    if new_input_size < int(max_position):
                        ...
                        add_padding = torch.tensor([padding] * padding_size).cuda(
                            device
                        )
            ...
            machine, human, persona, knowledge = tokenizer.convert_tokens_to_ids(
                SPECIAL_TOKENS
            )
            special_tokens_list = [
                machine,
            ...
            bleu += bleu_metric.compute(
                predictions=[pred_reply], references=[[gold_reply]]
            )["score"]
            ...
            # PG
            p_label_cvtd = torch.tensor(
                [1 if num in persona_grounding else 0 for num in range(5)],
                device=args.device,
            )

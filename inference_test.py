    ...
    for i in range(args.max_length):
        if model.config.model_type == "gpt2":
            if len(current_output) > 0:
                input_ids = torch.cat(
                    [gpt_input_ids, torch.tensor(current_output).type_as(input_ids)],
                    dim=-1,
                ).unsqueeze(0)
                token_type_ids = torch.cat(
                    [
                        gpt_tti,
                        torch.tensor(machine * len(current_output)).type_as(
                            token_type_ids
                        ),
                    ]
                ).unsqueeze(0)
            ...
            if len(current_output) > 0:
                decoder_input_ids = torch.cat(
                    [
                        bart_decoder_first_token,
                        torch.tensor(current_output).type_as(input_ids),
                    ],
                    dim=-1,
                ).unsqueeze(0)
...
def run():
    ...
    parser.add_argument(
        "--filename", type=str, default="", help="File name for saving output"
    )
        ...
        with torch.no_grad():
            ...
            for data_index, test_data in enumerate(test_loader):
                ...
                if model.config.model_type == "gpt2":
                    ...
                    machine, human, persona, knowledge, padding, bos = (
                        50257,
                        50258,
                        50259,
                        50260,
                    ...
                    for batch_i in range(args.test_batch_size):
                        ...
                        knowledge_pred = knowledge_candidates[batch_i][
                            knowledge_pred_idx
                        ]
                    ...
                    for batch_i in range(args.test_batch_size):
                        ...
                        mask_only_dial_tti_batch = torch.ne(
                            only_dial_token_type_ids_batch, padding
                        )
                        ...
                        if len(all_persona_pred[batch_i]) > 0:
                            concat_persona = torch.cat(
                                all_persona_pred[batch_i], dim=-1
                            )
                            new_persona = torch.cat(
                                [persona_tensor, concat_persona], dim=-1
                            )
                            new_persona_tti = torch.tensor(
                                [persona] * (new_persona.size()[0])
                            ).cuda(device)
                        ...
                        new_knowledge_tti = torch.tensor(
                            [knowledge] * (new_knowledge.size()[0])
                        ).cuda(device)

                        only_dial_input_ids_batch = only_dial_input_ids_batch[1:-1]
                        only_dial_token_type_ids_batch = only_dial_token_type_ids_batch[
                            1:
                        ]
                    ...
                    out_ids = sample_sequence(
                        final_input_tensor,
                        token_type_ids=final_input_tti_tensor,

...
class GPT2PK_ctxt(GPT2LMHeadModel):
    ...
    def forward(
        ...
        if input_eos is not None:
            lm_hidden_states = self.transformer(
            ...
            if persona_input_ids is not None:
                persona_emb = self.transformer(input_ids=persona_input_ids)[
                    "last_hidden_state"
                ]
            ...
            if knowledge_input_ids is not None:
                knowledge_emb = self.transformer(input_ids=knowledge_input_ids)[
                    "last_hidden_state"
                ]
                ...
                new_lm_label = torch.cat(
                    [
                        torch.tensor(
                            ...
                            * (
                                new_input_size
                                - (only_dial_input_ids_batch.size()[0])
                                + 1
                            )
                        ).cuda(device),

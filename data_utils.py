...
def choose_knowledge(knowledge, question):
    """
    считает tf-idf для токенов в базе знаний
    потом считает похожесть вопроса на каждое предложение из базы знаний
    возвращает топ самых похожих.
    ---
    почему бы не использовать sentence bert c той же идеей топа предложений?
    почему бы не кодировать все предложения в вектор и применять уже к ним attention в процессе обучения?
    """
    table = TfIdf()
...
def build_input_from_segments_bart_inctxt(
    ...
    # помечаем каждый четный токен как токен человека, каждый нечетный как токен машины
    history = [
    ...
    if inference == False:
        if len(knowledge) > 1:
            # получаем топ самых похожих предложений из базы знаний на вопрос
    ...
    for para in chosen_knowledge:
        ...
        # зачем условие на 100, если можно всегда делать срез?
        if len(para) > 100:
            short_para = para[:100]
        else:
            short_para = para
    ...
    for dataset_name, dataset in all_dataset.items():
        ...
        for dialog in dataset:
            ...
            for i, utt in enumerate(utterance):
                ...
                if model_name in ["GPT2", "transformer-decoder"]:
                ...
                elif model_name in ["BART", "transformer-encdec"]:
